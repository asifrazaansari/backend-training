...
const createOrder = async function (req, res) {
    ...
    if (!(userId && productId)) {
    ...
    } else if (data.isFreeAppUser === true.toString()) {
        data.amount = 0
        const allUser = await OrderModel.create(data)
        res.send({ msg: allUser })
    } else if (userId.balance < productId.price && data.isFreeAppUser === false.toString()) {
        ...
        return res.send({status: false, msg: "Please check the logic once again"})
    }
}
